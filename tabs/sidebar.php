...
<!-- sidebar: style can be found in sidebar.less -->
                ...
                if (count($resultArray) > 0) {
                     foreach ($resultArray as $row) {
                               ...
                               // echo $menu_name;
                                  ...
                                  $children = execQuery("select count(*) from NIC_PRMSSNS where child_of ='$row[PRMS_ID]' ",true);
 ...
 '.$menu_icon.' <span>'.' '.$menu_name . '</span> </i></a>';
                                       ...
                                       // echo $approval_title;
                                        ...
                                        $strQuery = "SELECT MENU_NAME,URL,PRMS_ID,ICON_TAG FROM NIC_PRMSSNS  WHERE PRMS_ID IN (SELECT PERMISSION_ID_FK FROM NIC_PRMSSN_MAP WHERE ROLE_ID_FK IN (SELECT ROLE_ID_FK FROM NIC_ROLE_MAP WHERE USER_ID_FK = " . $_SESSION['user_id'] . ")) AND MENU_LEVEL = 2 AND IS_MENU = 1 AND MODULE = '$item' AND CHILD_OF = '" . $row["PRMS_ID"] . "' ORDER BY MENU_POS ASC";
