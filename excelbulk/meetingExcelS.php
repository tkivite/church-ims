...
//RESPECTIVE attendance
    ...
    $sqltotalpr = "select 
				  sum(
				  (select (case when '$meetingtype'='DayOnly' then lunchDinnerRate when '$meetingtype'='HalfBoard' then dinnerRate else amount end) from perdiem where id=C.designationid)*(select (case when count(*)>0 then count(*)+$didheattendquery2 else count(*) end) from attendance A where A.MSISDN=R.msisdn and verification_status='Approved' and processing_status='0' and meeting_code='" . $_GET['cell'] . "')-0
				  )as 'Total'
    ...
    $result2 = @mysql_db_query($database, "select 
				  (select concat(firstname,' ',middlename,' ',lastname) from recipient where id=R.recipient_id)Name,
				   (select title from county where county.id=C.countyid) as `NameofCounty`,
				    ...
				    (select designation from perdiem where id=C.designationid)Designation,
				    (select title from jobgroup where id=C.jobgroupid)JobGroup,
				  $didheattendquery 
				  R.MSISDN as 'Telephone Number',
				  (select (case when '$meetingtype'='DayOnly' then lunchDinnerRate when '$meetingtype'='HalfBoard' then dinnerRate else amount end) from perdiem where id=C.designationid)Rate,
				  (select (case when '$meetingtype'='DayOnly' then lunchDinnerRate when '$meetingtype'='HalfBoard' then dinnerRate else amount end) from perdiem where id=C.designationid)*(select (case when count(*)>0 then count(*)+arrivalday else count(*) end) from attendance A where A.MSISDN=R.msisdn and verification_status='Approved' and processing_status='0' and meeting_code='" . $_GET['cell'] . "') as Amount,
				  0 as 'Less lunch',
				  0 as 'Transport',
				  0 as 'Extra per diem',
				  0 as 'Others',
				  (
				  (select (case when '$meetingtype'='DayOnly' then lunchDinnerRate when '$meetingtype'='HalfBoard' then dinnerRate else amount end) from perdiem where id=C.designationid)*(select (case when count(*)>0 then count(*)+arrivalday else count(*) end) from attendance A where A.MSISDN=R.msisdn and verification_status='Approved' and processing_status='0' and meeting_code='" . $_GET['cell'] . "')-0
...
//RESPECTIVE attendance
     ...
     $sqltotalpr = "select 
				  sum(
				  (select amount from perdiem where id=C.designationid)*(select (case when count(*)>0 then count(*)+$travelling_days else count(*) end) from attendance A where A.MSISDN=R.msisdn and meeting_code='" . $_GET['cell'] . "')-
   ...
   $query_4 = "select 
				  ...
				  $didheattendquery 
				  R.MSISDN as 'Telephone Number',
				  (select amount from perdiem where id=C.designationid)Rate,
				 (SELECT sum(credit) FROM temp_supervision_details where title='Per Diem' and apiorder_id in $attendanceids $limit) as Amount,
				  (SELECT sum(credit) FROM temp_supervision_details where title='Lunch' and apiorder_id in $attendanceids $limit) as 'Less_lunch_day_500',
				  (SELECT sum(credit) FROM temp_supervision_details where title='Transport/Fuel' and apiorder_id in $attendanceids $limit) as 'Transport',
				  (SELECT sum(credit) FROM temp_supervision_details where title='Extra Per Diem' and apiorder_id in $attendanceids $limit) as 'Extra_per_diem',
